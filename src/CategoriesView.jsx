...
export default function CategoriesView({ categories, onSaveCategories }) {
  ...
  const saveCategory = () => {
    ...
    setForm({ label: '', color: '#888888', parentId: '', isIncome: false, target: '' });
  };
  ...
  const saveInlineEdit = (id) => {
    ...
    const newCats = categories.map(c => c.id === id ? { ...c, ...catData } : c);
  ...
  const startInlineEdit = (cat) => {
    setEditingId(cat.id);
    setEditForm({ label: cat.label, color: cat.color, parentId: cat.parentId || '', isIncome: cat.isIncome || false, target: cat.target?.toString() || '' });
  };
      ...
      <div style={{ display:'grid', gridTemplateColumns:'280px 1fr', gap:20, alignItems: 'start' }}>
        <div style={{ background:'var(--color-background-primary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:'var(--border-radius-lg)', padding:16, position: 'sticky', top: 76 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <div><label style={{ fontSize:12, color:'var(--color-text-secondary)', marginBottom:4, display:'block' }}>Name:</label><input className="input-f" value={form.label} onChange={e => setForm({...form, label: e.target.value})} placeholder="e.g., Groceries" /></div>
            <div><label style={{ fontSize:12, color:'var(--color-text-secondary)', marginBottom:4, display:'block' }}>Target ($):</label><input className="input-f" type="number" value={form.target} onChange={e => setForm({...form, target: e.target.value})} placeholder="500" /></div>
            <div><label style={{ fontSize:12, color:'var(--color-text-secondary)', marginBottom:4, display:'block' }}>Color:</label><div style={{ display:'flex', gap:8 }}><input type="color" value={form.color} onChange={e => setForm({...form, color: e.target.value})} style={{ width:40, height:34, border:'none', background:'none', cursor:'pointer' }} /><input className="input-f" value={form.color} onChange={e => setForm({...form, color: e.target.value})} /></div></div>
            <div><label style={{ fontSize:12, color:'var(--color-text-secondary)', marginBottom:4, display:'block' }}>Parent:</label><select className="input-f" value={form.parentId} onChange={e => setForm({...form, parentId: e.target.value})}><option value="">None</option>{parents.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}</select></div>
            {!form.parentId && <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:4 }}><label className="switch" style={{ width:30, height:16 }}><input type="checkbox" checked={form.isIncome} onChange={e => setForm({...form, isIncome: e.target.checked})} /><span className="slider" style={{ borderRadius:16 }}></span></label><span style={{ fontSize:12, color:'var(--color-text-secondary)' }}>Is Income?</span></div>}
                ...
                <div className="txn-row" style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px' }}>
                  ...
                  {isEditingP ? (
                    <div style={{ display:'flex', gap:8, alignItems:'center', flex:1 }}>
                      ...
                      <input className="input-f" style={{ flex:1, fontSize:13, padding:'4px 8px' }} type="number" value={editForm.target} onChange={e => setEditForm({...editForm, target: e.target.value})} placeholder="Target" />
                      <label className="switch" style={{ width:24, height:14 }} title="Is Income?"><input type="checkbox" checked={editForm.isIncome} onChange={e => setEditForm({...editForm, isIncome: e.target.checked})} /><span className="slider"></span></label>
                    </div>
                  ) : (
                    <>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        {p.target > 0 && <div style={{ fontSize:11, color:'var(--color-text-secondary)' }}>Target: {fmt(p.target)}</div>}
                      </div>
                    </>
                  )}
                  <div style={{ display:'flex', gap:8 }}>
                    {isEditingP ? (
                      ...
                      <>
                        ...
                        <button className="btn-g" style={{ padding:'4px 10px', fontSize:12, color:'var(--color-text-danger)' }} onClick={() => del(p.id)}>✕</button>
                      </>
                    )}
                  </div>
                </div>
                    ...
                    <div key={c.id} className="txn-row" style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 16px 10px 40px', background:'var(--color-background-secondary)', borderTop:'0.5px solid var(--color-border-tertiary)' }}>
                      {isEditingC ? (
                        <div style={{ display:'flex', gap:8, alignItems:'center', flex:1 }}>
                          ...
                          <input className="input-f" style={{ flex:1, fontSize:12, padding:'3px 6px' }} type="number" value={editForm.target} onChange={e => setEditForm({...editForm, target: e.target.value})} placeholder="Target" />
                          <select className="input-f" style={{ flex:1, fontSize:12, padding:'3px 6px' }} value={editForm.parentId} onChange={e => setEditForm({...editForm, parentId: e.target.value})}>
                            ...
                            {parents.filter(p => p.id !== c.id).map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
                          </select>
                        </div>
                      ) : (
                        <>
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            {c.target > 0 && <div style={{ fontSize:10, color:'var(--color-text-secondary)' }}>Target: {fmt(c.target)}</div>}
                          </div>
                        </>
                      )}
                      <div style={{ display:'flex', gap:8 }}>
                        {isEditingC ? (
                          ...
                          <>
                            ...
                            <button className="btn-g" style={{ padding:'3px 8px', fontSize:11, color:'var(--color-text-danger)' }} onClick={() => del(c.id)}>✕</button>
                          </>
                        )}
                      </div>
                    </div>
